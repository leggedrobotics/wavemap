#ifndef WAVEMAP_CORE_INTEGRATOR_PROJECTION_MODEL_IMPL_LIDAR_CHANNEL_PROJECTOR_INL_H_
#define WAVEMAP_CORE_INTEGRATOR_PROJECTION_MODEL_IMPL_LIDAR_CHANNEL_PROJECTOR_INL_H_
...
namespace wavemap {
inline SensorCoordinates LidarChannelProjector::cartesianToSensor(
...
inline Point3D LidarChannelProjector::sensorToCartesian(
...
inline FloatingPoint LidarChannelProjector::imageOffsetToErrorSquaredNorm(
  ...
  // for the change in density along the azimuth axis in function of
  // elevation. Purely a function of the continuous angle, so this is
  // unaffected by elevation's irregular spacing.
...
LidarChannelProjector::imageOffsetsToErrorSquaredNorms(
...
inline ImageCoordinates LidarChannelProjector::cartesianToImage(
    const Point3D& C_point) const {
...
inline FloatingPoint LidarChannelProjector::cartesianToSensorZ(
    const Point3D& C_point) const {
  return C_point.norm();
}
}  // namespace wavemap

#endif  // WAVEMAP_CORE_INTEGRATOR_PROJECTION_MODEL_IMPL_LIDAR_CHANNEL_PROJECTOR_INL_H_
